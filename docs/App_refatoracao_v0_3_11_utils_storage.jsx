...
import { LS_PREFIX, BACKUP_SCHEMA_VERSION, BACKUP_STORAGE_KEYS } from "./constants/storageKeys.js";
...
const normText = (value) => String(value || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/\s+/g, " ").trim();
...
const AUTO_CATEGORY_STOPWORDS = new Set([
...
const autoCatTokens = (text) => normText(text)
  .split(/[^a-z0-9]+/)
  .filter(token => token.length >= 4 && !AUTO_CATEGORY_STOPWORDS.has(token) && !/^\d+$/.test(token));

const resolveAutoCatId = (cats, ids = [], labels = [], fallback = "cat10") => {
...
const scoreAutoCategoryRule = (descNorm, rule) => {
  let score = 0;
...
const guessCatFromHistory = (desc, tipo, trans, cats) => {
  const tokens = autoCatTokens(desc);
  if (!tokens.length) return null;
  const descNorm = normText(desc);
  const validIds = new Set(flattenCats(cats).filter(f => !f.hasSubs).map(f => f.id));
...
export default function App() {
  ...
  const guessCat=(desc,tipo="despesa")=>{
  ...
  const filteredTransactions = useMemo(() => {
    const hasDateRange = Boolean(transactionFilters.dataInicio || transactionFilters.dataFim);
    const baseRows = hasDateRange ? trans : monthTrans;
    return baseRows
      .filter(t => !transactionFilters.dataInicio || (t.data || "") >= transactionFilters.dataInicio)
      .filter(t => !transactionFilters.dataFim || (t.data || "") <= transactionFilters.dataFim)
      .filter(t => !transactionFilters.rootCatId || findRootCat(cats, t.catId)?.id === transactionFilters.rootCatId)
      .filter(t => !transactionFilters.origem || t.origem === transactionFilters.origem)
      .filter(t => !transactionFilters.tipo || t.tipo === transactionFilters.tipo)
      .filter(t => !transactionFilters.status || t.status === transactionFilters.status)
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            ...
            <div style={card()}>
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:10, alignItems:"end" }}>
                <div><div style={lbl}>Data início</div><DateInput style={inp} value={transactionFilters.dataInicio} onChange={value=>setTransactionFilters(f=>({...f,dataInicio:value}))}/></div>
                <div><div style={lbl}>Data fim</div><DateInput style={inp} value={transactionFilters.dataFim} onChange={value=>setTransactionFilters(f=>({...f,dataFim:value}))}/></div>
                <div><div style={lbl}>Categoria principal</div><select style={inp} value={transactionFilters.rootCatId} onChange={e=>setTransactionFilters(f=>({...f,rootCatId:e.target.value}))}><option value="">Todas</option>{rootCats.map(c=><option key={c.id} value={c.id}>{c.icon} {c.nome}</option>)}</select></div>
                <div><div style={lbl}>Origem</div><select style={inp} value={transactionFilters.origem} onChange={e=>setTransactionFilters(f=>({...f,origem:e.target.value}))}><option value="">Todas</option><option value="corrente">Conta corrente</option><option value="cartao">Cartão</option><option value="vale_alimentacao">Vale alimentação</option><option value="vale_refeicao">Vale refeição</option></select></div>
                <div><div style={lbl}>Tipo</div><select style={inp} value={transactionFilters.tipo} onChange={e=>setTransactionFilters(f=>({...f,tipo:e.target.value}))}><option value="">Todos</option><option value="receita">Receita</option><option value="despesa">Despesa</option></select></div>
                <div><div style={lbl}>Status</div><select style={inp} value={transactionFilters.status} onChange={e=>setTransactionFilters(f=>({...f,status:e.target.value}))}><option value="">Todos</option><option value="previsto">Previsto</option><option value="parcial">Parcial</option><option value="pago">Pago</option></select></div>
                <button onClick={()=>setTransactionFilters({ dataInicio:"", dataFim:"", rootCatId:"", origem:"", tipo:"", status:"" })} style={ghost({ height:38 })}>Limpar filtros</button>
              </div>
            </div>
