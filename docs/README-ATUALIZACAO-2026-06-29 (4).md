...
## Atualização complementar — v0.3.11

A versão `v0.3.11` consolida, em uma única entrega, as refatorações técnicas previstas entre v0.3.8 e v0.3.11.

### Refatorações aplicadas

- `src/utils/moneyUtils.js`: centraliza formatação, máscara e conversão monetária.
- `src/utils/dateUtils.js`: centraliza utilitários de data, mês, competência e data brasileira.
- `src/constants/storageKeys.js`: centraliza constantes de LocalStorage e backup.
- `src/hooks/useLocalStorage.js`: centraliza `lsGet`, `lsSave` e `useLS`.
- `src/components/ui/DateInput.jsx`: passa a reaproveitar `dateUtils.js`.
...
- Versão visual atualizada para `v0.3.11`.
...
- Criar lançamento com valor e data.
- Filtrar lançamentos por data inicial/final.
- Criar compra parcelada.
- Criar simulação com valor.
- Registrar dívida e amortização.
- Criar despesa compartilhada.
- Exportar backup.
- Restaurar backup.
- Recarregar a aplicação e confirmar dados preservados.
